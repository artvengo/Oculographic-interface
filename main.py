...
class VideoObjectDetector:
    ...
    def _alternative_camera_detection(self, camera_id=0, detect_qr=True):
        ...
        print("\n⚠️ Вся информация будет отображаться прямо на видео")
        ...
        while True:
            ...
            if frame_count % detection_frame_interval == 0:
                try:
                    ...
                    print(f"⚠️ Ошибка детекции на кадре {frame_count}: {e}")
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                print("⏹️ Обработка прервана пользователем")
                break
...
def main():
    ...
    detector = VideoObjectDetector(model_type="tiny-yolov3")
